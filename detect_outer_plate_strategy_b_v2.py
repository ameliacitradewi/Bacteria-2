...
def transform_annotations(
    ...
    """Transformasikan bounding box asli ke koordinat crop ternormalisasi.

    Merge dilakukan melalui kolom helper string, sehingga tidak bergantung
    pada dtype ``image_id`` yang diinferensikan pandas dari masing-masing CSV.
    """
    ...
    if "image_id" not in detections.columns:
        raise KeyError(
            "Kolom 'image_id' tidak ditemukan pada tabel hasil deteksi."
        )

    objects = objects.copy()
    detections = detections.copy()

    # Helper key selalu berisi Python string atau None di kedua tabel.
    objects["_image_id_key"] = objects["image_id"].map(
        _normalize_image_id_value
    )
    detections["_image_id_key"] = detections["image_id"].map(
        _normalize_image_id_value
    )
    ...
    # Saat --limit digunakan, proses hanya anotasi untuk gambar pada run ini.
    detected_ids = set(detections["_image_id_key"].tolist())
    objects = objects[objects["_image_id_key"].isin(detected_ids)].copy()
    if objects.empty:
        print(
            "Tidak ada anotasi objek yang cocok dengan image_id hasil deteksi."
        )
        return
    ...
    missing_detection_columns = [
        column for column in columns if column not in detections.columns
    ]
    if missing_detection_columns:
        raise KeyError(
            "Kolom hasil deteksi yang dibutuhkan tidak tersedia: "
            f"{missing_detection_columns}"
        )
    ...
    numeric_columns = [
        "x", "y", "width", "height",
        "crop_x0", "crop_y0", "normalization_scale",
    ]
    ...
    merged.loc[valid, "width_normalized"] = (
        merged.loc[valid, "width"]
        * merged.loc[valid, "normalization_scale"]
    )
    merged.loc[valid, "height_normalized"] = (
        merged.loc[valid, "height"]
        * merged.loc[valid, "normalization_scale"]
    )

    # Periksa pusat setiap bounding box terhadap counting mask 2048x2048.
    for _, detection in detections.iterrows():
        ...
        relative_mask_path = detection.get(
            "normalized_counting_mask_path"
        )
        ...
        centers_x = (
            merged.loc[group_indices, "x_normalized"]
            + merged.loc[group_indices, "width_normalized"] / 2
        )
        centers_y = (
            merged.loc[group_indices, "y_normalized"]
            + merged.loc[group_indices, "height_normalized"] / 2
        )

        xs = np.rint(centers_x.to_numpy()).astype(np.int64)
        ys = np.rint(centers_y.to_numpy()).astype(np.int64)
        ...
        inside_mask[inside_bounds] = (
            mask[ys[inside_bounds], xs[inside_bounds]] > 0
        )

        merged.loc[
            group_indices, "center_inside_counting_mask"
        ] = inside_mask
